...
class PIIStore:
    ...
    async def get_persons_batch(
        self, db: AsyncSession, external_ids: list[str]
    ) -> dict[str, PIIPerson]:
    ...
    async def update_person_key(
        self, db: AsyncSession, external_id: str, person_key: str
    ) -> None:
